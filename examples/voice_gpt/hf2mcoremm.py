...
def convert_checkpoint_from_transformers_to_megatron(args):
    ...
    for k, v in megatron_args.items():
        setattr(margs, k, v)
    from speech_llama import SpeechLlamaForCausalLM
    state_dict = SpeechLlamaForCausalLM.from_pretrained(args.load_path).state_dict()
    internal_state_dict = {}
    ...
    projection_weight = internal_state_dict["vision_projection.encoder.weight"]
    projection_bias = internal_state_dict["vision_projection.encoder.bias"]
    out_projection_weight = torch.chunk(projection_weight, args.target_tensor_model_parallel_size, dim=0)
    out_projection_bias = torch.chunk(projection_bias, args.target_tensor_model_parallel_size, dim=0)
    ...
    for pp_rank in range(args.target_pipeline_model_parallel_size):
        ...
        if pp_rank == args.target_pipeline_model_parallel_size - 1:
            ...
            for i in range(args.target_tensor_model_parallel_size):
                params_dict = get_element_from_dict_by_path(output_state_dict[i], "model")
                params_dict["vision_projection.encoder.weight"] = out_projection_weight[i].clone()
                params_dict["vision_projection.encoder.bias"] = out_projection_bias[i].clone()
        ...
        for tp_rank in range(args.target_tensor_model_parallel_size):
            ...
            for key in params_dict.keys():
                if key.startswith('vision_projection'):
                    params_dict_new[key] = params_dict[key]
                else:
