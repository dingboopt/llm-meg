...
def convert_checkpoint_from_transformers_to_megatron(args):
    ...
    for layer_id in range(config.num_hidden_layers):
        ...
        q_bias_key = 'model.layers.' + str(layer_id) + '.self_attn.q_proj.bias'
        qkv_bias_enable = q_bias_key in state_dict.keys()
        if qkv_bias_enable:
    ...
    for pp_rank in range(args.target_pipeline_model_parallel_size):
        ...
        for layer in range(num_layers):
            ...
            for i in range(args.target_tensor_model_parallel_size):
                ...
                if qkv_bias_enable:
                    self_attn_query_name = f"decoder.layers.{layer}.self_attn.query.bias"
                    query_bias = params_dict[self_attn_query_name]
                    del params_dict[self_attn_query_name]
                    self_attn_kv_name = f"decoder.layers.{layer}.self_attn.key_value.bias"
                    kv_bias = params_dict[self_attn_kv_name]
                    del params_dict[self_attn_kv_name]

                    # torch.Size([8 512, 4096])
                    group_query_bias = query_bias.view(num_groups // args.target_tensor_model_parallel_size, num_heads // num_groups * hidden_size_per_head)
                    # torch.Size(8, 256, 4096])
                    group_kv_bias = kv_bias.view(num_groups // args.target_tensor_model_parallel_size, 2 * hidden_size_per_head)
        ...
        # adapt for multi-modal
        for tp_rank in range(args.target_tensor_model_parallel_size):
            params_dict = get_element_from_dict_by_path(output_state_dict[tp_rank], "model")
            params_dict_new={}
            for key in params_dict.keys():
                params_dict_new['language_model.'+key] = params_dict[key]
            output_state_dict[tp_rank]["model"] = params_dict_new
...
def convert_checkpoint_from_megatron_to_transformers(args):
    ...
    for tp_rank in range(tp_size):
        embeddings = tp_state_dicts[tp_rank]["model"]["language_model.embedding.word_embeddings.weight"]
    ...
    encoder_bias = []
    for tp_rank in range(tp_size):
        weight = tp_state_dicts[tp_rank]["model"]["vision_projection.encoder.weight"]
        encoder_weight.append(weight)
        bias = tp_state_dicts[tp_rank]["model"]["vision_projection.encoder.bias"]
        encoder_bias.append(bias)
    encoder_weight = torch.cat(encoder_weight)
    encoder_bias = torch.cat(encoder_bias)
    output_state_dict["model.speech_projector.weight"] = encoder_weight.clone()
    output_state_dict["model.speech_projector.bias"] = encoder_bias.clone()
    
    
    
    #internal_state_dict["vision_projection.encoder.weight"] = state_dict["model.speech_projector.weight"]
    #internal_state_dict["vision_projection.encoder.bias"] = state_dict["model.speech_projector.bias"]
    # Transformer Layers
    print("Converting transformer layers")
    # The number of heads.
    ...
    for pp_rank in range(pp_size):
        ...
        for key, val in get_element_from_dict_by_path(tp_state_dicts[0], path).items():
            if key.endswith('_extra_state') or 'vision_projection' in key:
                continue
            if 'linear_fc' in key and 'norm' not in key:
                print(key)
                key_list = key.split('.')
                layer_id = int(key_list[3]) + pp_rank * num_layers
            ...
            new_key = key.replace('language_model.decoder.', '')
            ...
            if op_name.endswith("layer_norm_weight") or op_name.endswith("layernorm"):
                ln_name = "input_layernorm" if 'qkv' in op_name else "post_attention_layernorm"
                #print(f'### in name {ln_name}')
            ...
            # Transpose the QKV matrix.
                ...
                all_qkvs = [i.reshape(num_groups // args.target_tensor_model_parallel_size, (heads // num_groups * hidden_size_per_head + 2 * hidden_size_per_head)) for i in torch.chunk(params, args.target_tensor_model_parallel_size, 0)]
                split_size = heads // num_groups * hidden_size_per_head
    ...
    try:
        output_state_dict["model.norm.weight"] = params["language_model.decoder.final_layernorm.weight"].to(dtype).clone()
    except:
        output_state_dict["model.norm.weight"] = params["language_model.decoder.final_norm.weight"].to(dtype).clone()
